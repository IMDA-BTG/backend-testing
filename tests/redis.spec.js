...
// baseDir = aiverify repo base dir
const baseDir = process.env.BASEDIR ?? "/app/aiverify"
// testFileDir = test files folder in backend-testing repo
let testFileDir = "./uploads"
// Portal uploads folder
let uploadsDir = process.env.UPLOADSDIR
...
test.describe('Test Engine Task', () => {
    ...
    test.beforeAll(() => {
        // Copy test files to portal uploads folder
        if (uploadsDir) {
            console.log("Copy test files to portal uploads dir...")
            fs.cpSync(testFileDir, uploadsDir, { recursive: true })
        }
    })
    ...
    test('Invalid Input Schema (Upload)', async () => {
        ...
        task = JSON.stringify({
            ...
            "testDataset": baseDir + "/uploads/data/pickle_pandas_tabular_loan_testing.sav",
            "modelFile": baseDir + "/uploads/model/combine_all.sh",
            "groundTruthDataset": baseDir + "/uploads/data/pickle_pandas_tabular_loan_testing.sav",
            "modelType": "classification",
            "groundTruth": "Interest_Rate"
        })
